...
EOF`
	...
	installKubeletYum         = `sudo yum -y install kubelet-1.14.2-0`
	installNFSUtilsYum        = `sudo yum -y install nfs-utils` // required for the kubelet
	installKubectlYum         = `sudo yum -y install kubectl-1.14.2-0`
	...
	installKubeletApt       = `sudo apt-get -y install kubelet=1.14.2-00`
	stopKubeletService      = `sudo systemctl stop kubelet`
	installNFSCommonApt     = `sudo apt-get -y install nfs-common`
	installKubectlApt       = `sudo apt-get -y install kubectl=1.14.2-00`
