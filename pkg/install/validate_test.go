...
func TestDisconnectedInstallationPrereq(t *testing.T) {
	...
	valid, _ := disconnectedInstallation{cluster: p.Cluster, registryProvided: p.ConfgiureDockerWithPrivateRegistry()}.validate()
	if valid {
		t.Errorf("expected invalid, but got valid")
	}

	p.DockerRegistry.SetupInternal = true
	valid, errs := disconnectedInstallation{cluster: p.Cluster, registryProvided: p.ConfgiureDockerWithPrivateRegistry()}.validate()
	if !valid {
		t.Errorf("expected valid, but got invalid")
		fmt.Println(errs)
	}
